  ...
  ({
    ...
    const rootClassName = useMemo(
      () =>
        [
          'repoloop',
          isVertical ? 'repoloop--vertical' : 'repoloop--horizontal',
          fadeOut && 'repoloop--fade',
          scaleOnHover && 'repoloop--scale-hover',
          className
        ]
          .filter(Boolean)
          .join(' '),
    ...
    const renderLogoItem = useCallback(
      (item, key) => {
        if (renderItem) {
          return (
            <li className="repoloop__item" key={key} role="listitem">
              {renderItem(item, key)}
            </li>
          );
        }
        ...
        const content = isNodeItem ? (
          <span className="repoloop__node" aria-hidden={!!item.href && !item.ariaLabel}>
            {item.node}
          </span>
        ) : (
          <img
...
RepoLoop.displayName = 'RepoLoop';

export default RepoLoop;
